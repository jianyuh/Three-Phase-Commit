		...
		while (!killSig) {
			try {
				IncomingSock incomingSock = new IncomingSock(
						serverSock.accept());				//ServerSocket.accept() will return a Socket, as the input parameter of IncomingSock
				socketList.add(incomingSock);
				incomingSock.start();						//This is exactly how to use Thread, start a new listening thread for socket(TCP server IP, server port, Client IP, Client port)
				conf.logger.fine(String.format(					//I shall learn how to use logger....fine....
						"Server %d: New incoming connection accepted from %s",
						procNum, incomingSock.sock.getInetAddress()
								.getHostName()));						// record the sock getHostName? I have to test myself?? I guess it returns a IP addr
			} catch (IOException e) {
				if (!killSig) {
